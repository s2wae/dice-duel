...
func _ready() -> void:
	assert(target, "No available target to drag!")
...
func _on_target_input_event(_viewport: Node, event: InputEvent) -> void:
	...
	if dragging and event.is_action_pressed("rmb"):
		_cancel_dragging()
	elif not dragging and event.is_action_pressed("lmb"):
		_start_dragging()
	elif dragging and event.is_action_released("lmb"):
		_drop()
