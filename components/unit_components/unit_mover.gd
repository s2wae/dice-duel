...
@export var play_areas: Array[UnitPlayArea]
...
func _set_highlighters(enabled: bool) -> void:
	for play_area: UnitPlayArea in play_areas:
...
func _move_unit(unit: Unit, play_area: UnitPlayArea, tile: Vector2i) -> void:
	play_area.unit_grid.add_unit(tile, unit)
	unit.global_position = play_area.get_global_from_tile(tile) - Main.QUARTER_CELL_SIZE
